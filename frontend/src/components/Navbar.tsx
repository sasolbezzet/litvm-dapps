      ...
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center gap-2.5 no-underline">
            ...
            <span className="text-base font-bold tracking-tight">LitVM DeFi</span>
          </Link>
    ...
    return (
      <div className="relative" ref={ref}>
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2.5 bg-zinc-800/50 border border-zinc-700 rounded-xl px-3.5 py-2 hover:border-zinc-600 transition-colors">
          ...
          {balance && <span className="text-xs text-zinc-500">{Number(formatUnits(balance.value, balance.decimals)).toFixed(3)} {balance.symbol}</span>}
        </button>
        {open && (
          <div className="absolute right-0 top-12 w-64 rounded-xl bg-zinc-900 border border-zinc-700 shadow-2xl overflow-hidden z-50">
            <div className="p-3 border-b border-zinc-800">
              <div className="text-xs font-mono text-zinc-300">{address.slice(0, 10)}...{address.slice(-8)}</div>
              <div className="text-xs text-zinc-500 mt-1">{chainId === 4441 ? "LitVM" : "Sepolia"} · Chain {chainId}</div>
            </div>
            <button onClick={() => { disconnect(); setOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-red-400 hover:bg-zinc-800 transition-colors">Disconnect</button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-12 w-64 rounded-xl bg-zinc-900 border border-zinc-700 shadow-2xl overflow-hidden z-50">
          <div className="p-2">
            <button onClick={() => { connect({ connector: injected() }); setOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium hover:bg-zinc-800 transition-colors">
