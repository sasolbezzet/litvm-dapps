...
const config = createConfig({
  ...
  transports:{[litvm.id]:http(LITVM_RPC),[sepolia.id]:http(SEPOLIA_RPC)},
  ssr:false,
});
...
export default function RootLayout({children}:{children:React.ReactNode}){
  return <html lang="en"><body>
    <WagmiProvider config={config as any}><QueryClientProvider client={qc}><Navbar/><main className="max-w-6xl mx-auto px-4 pb-24">{children}</main></QueryClientProvider></WagmiProvider>
  </body></html>;
}
